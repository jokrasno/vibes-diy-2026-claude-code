...
def make_level(filename: str, data: bytes, idx: int, total: int) -> dict[str, Any]:
    ...
    is_boss = idx == total - 1
    suffix = random.choice(["Boss Room", "Dungeon", "Wilds", "Vault"])
    if idx == 0:
        suffix = random.choice(["Village", "Studio", "Workshop", "Atrium"])
    quest_variants = [
        "Collect the generated quest item and reach the portal.",
        "Recover the relic and escape through the exit portal.",
        "Find the glowing item, then push on to the portal.",
    ]
    mood = random.choice(["glitchy", "haunted", "glowing", "dreamy", "stormy"])
    guardian = random.choice(["Guardian", "Warden", "Sentinel", "Watcher"])
    guide = random.choice(["Guide", "Sage", "Helper", "Scout"])
    return {
        "id": f"photo-{idx + 1}",
        "name": f"{name} {suffix}",
        "theme": random.choice(THEMES),
        "summary": f"A {mood} room inspired by {name}, with loot, NPC/enemy, and an exit portal.",
        "quest": random.choice(quest_variants) if not is_boss else "Defeat the generated photo boss and escape.",
        "grid": BASE_GRID,
...
def make_fallback_campaign(level_inputs: list[tuple[str, bytes]]) -> dict[str, Any]:
    ...
    title_prefix = random.choice(["CameraQuest", "PixelQuest", "LensRaid", "PhotoForge"])
    variant = secrets.token_hex(1).upper()
    return {
        "title": f"{title_prefix}: {title_seed} Campaign {variant}",
        "premise": random.choice([
            "Uploaded photos are converted into a connected top-down pixel RPG campaign.",
            "Your camera roll becomes a weird little fantasy dungeon chain.",
            "Every photo turns into a playable room with enemies, loot, and a boss.",
        ]),
        "victory": random.choice([
            "You stabilized the photo realms and escaped the camera roll.",
            "The photo dungeon collapsed behind you — you made it out alive.",
            "Reality reasserted itself and you returned with the loot.",
        ]),
        "levels": levels,
    }
